...
impl<'a> Admin<'a> {
    ...
    fn state(&self, storage: &'a dyn Storage) -> StdResult<AdminState> {
        Ok(self.0.may_load(storage)?.unwrap_or(AdminState::A(AdminUninitialized)))
    }
        ...
        C: Clone + Debug + PartialEq + JsonSchema,
    {
        ...
        Ok(Response::new()
            ...
            .add_attribute("proposed", res.proposed.unwrap_or_else(|| "None".to_string()))
...
mod tests {
    ...
    fn invalid_uninitialized_state_transitions() {
        ...
        let err =
            admin.update::<Empty, Empty>(deps.as_mut(), info.clone(), ClearProposed).unwrap_err();
        assert_eq!(err, AdminError::StateTransitionError {});

        let err =
            admin.update::<Empty, Empty>(deps.as_mut(), info.clone(), AcceptProposed).unwrap_err();
        assert_eq!(err, AdminError::StateTransitionError {});

        let err = admin.update::<Empty, Empty>(deps.as_mut(), info, AbolishAdminRole).unwrap_err();
        assert_eq!(err, AdminError::StateTransitionError {});
    }
    ...
    fn invalid_admin_set_no_proposed_state_transitions() {
        ...
        let err = admin.update::<Empty, Empty>(deps.as_mut(), info, AcceptProposed).unwrap_err();
        assert_eq!(err, AdminError::StateTransitionError {});
    }
    ...
    fn invalid_admin_role_abolished_state_transitions() {
        ...
        admin.initialize(mut_deps.storage, mut_deps.api, AdminInit::AbolishAdminRole).unwrap();
    ...
    fn clear_proposed_permissions() {
        ...
        let bad_guy = Addr::unchecked("doc_oc");
        let info = mock_info(bad_guy.as_ref(), &[]);
        let err = admin.update::<Empty, Empty>(deps.as_mut(), info, ClearProposed).unwrap_err();
    ...
    fn clear_proposed() {
        ...
        let mut_deps = deps.as_mut();
        admin.update::<Empty, Empty>(mut_deps, info, ClearProposed).unwrap();
    ...
    fn accept_proposed() {
        ...
        let mut_deps = deps.as_mut();
        admin.update::<Empty, Empty>(mut_deps, info, AcceptProposed).unwrap();
    ...
    fn abolish_admin_role() {
        ...
        let mut_deps = deps.as_mut();
        admin.update::<Empty, Empty>(mut_deps, info, AbolishAdminRole).unwrap();
