...
#[test]
fn test_pagination_on_deposit_caps_query_works() {
    ...
    let vaults_res = mock.query_deposit_caps(None, Some(58_u32));

    // Assert maximum is observed
    assert_eq!(vaults_res.len(), 30);

    let vaults_res = mock.query_deposit_caps(None, Some(2_u32));

    // Assert limit request is observed
    assert_eq!(vaults_res.len(), 2);

    let vaults_res_a = mock.query_deposit_caps(None, None);

    let vaults_res_b =
        mock.query_deposit_caps(Some(vaults_res_a.last().unwrap().vault.clone()), None);
    let vaults_res_c =
        mock.query_deposit_caps(Some(vaults_res_b.last().unwrap().vault.clone()), None);
    let vaults_res_d =
        mock.query_deposit_caps(Some(vaults_res_c.last().unwrap().vault.clone()), None);
    ...
    let combined = vaults_res_a
        ...
        .map(|config| config.vault.check(&MockApi::default()).unwrap())
