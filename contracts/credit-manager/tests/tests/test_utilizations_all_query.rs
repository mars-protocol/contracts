...
fn utilizations_are_zero() {
    ...
    let _ = res.iter().map(|vault_utilization| {
        assert_eq!(Uint128::zero(), vault_utilization.utilization.amount);
        assert_eq!(vault_info_1.deposit_cap.denom, vault_utilization.utilization.denom);
    });
}
...
fn utilizations_if_cap_is_base_denom() {
    ...
    let vault_utilization_1 = VaultUtilizationResponse {
        vault: vault_1.clone().into(),
        utilization: Coin {
            denom: vault_info_1.deposit_cap.denom.clone(),
            amount: Uint128::new(50),
        },
    };

    let vault_utilization_2 = VaultUtilizationResponse {
        vault: vault_2.clone().into(),
        utilization: Coin {
            denom: vault_info_2.deposit_cap.denom.clone(),
            amount: Uint128::zero(),
        },
    };

    assert_eq!(res, vec![vault_utilization_1, vault_utilization_2])
}
...
fn utilization_in_other_denom() {
    ...
    let user = Addr::unchecked("user");
    let mut mock = MockEnv::new()
        .set_params(&[jake_info.clone(), osmo_info.clone(), atom_info.clone()])
        .vault_configs(&[vault_info_1.clone(), vault_info_2.clone()])
        .fund_account(AccountToFund {
            addr: user.clone(),
