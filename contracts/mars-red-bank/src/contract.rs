...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::InitAssetTokenCallback {
            denom,
        } => execute_init_asset_token_callback(deps, env, info, denom),
        ...
        ExecuteMsg::UpdateUncollateralizedLoanLimit {
            ...
            execute_update_uncollateralized_loan_limit(deps, env, info, user_addr, denom, new_limit)
        }
        ...
        ExecuteMsg::Deposit {
            ...
            execute_deposit(deps, env, info, depositor_address, on_behalf_of, denom, deposit_amount)
        }
        ...
        } => execute_withdraw(deps, env, info, denom, amount, recipient_address),
        ExecuteMsg::Borrow {
            denom,
            amount,
            recipient: recipient_address,
        } => execute_borrow(deps, env, info, denom, amount, recipient_address),
        ...
        ExecuteMsg::Repay {
            ...
            execute_repay(deps, env, info, repayer_address, on_behalf_of, denom, repay_amount)
        }
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::UserAssetDebt {
            ...
            let address = deps.api.addr_validate(&user_address)?;
            to_binary(&query_user_asset_debt(deps, env, address, denom)?)
        }
        ...
        QueryMsg::UncollateralizedLoanLimit {
            ...
            to_binary(&query_uncollateralized_loan_limit(deps, user_address, denom)?)
        }
