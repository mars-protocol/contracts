...
mod tests {
    ...
    fn test_withdraw_native() {
        ...
        assert_eq!(
            res.events,
            vec![th_build_interests_updated_event("somecoin", &expected_params)]
        );
    ...
    fn test_borrow_and_repay() {
        ...
        let debt =
            DEBTS.load(&deps.storage, (cw20_contract_addr.as_bytes(), &borrower_addr)).unwrap();
    ...
    pub fn test_liquidate() {
        ...
        // Perform first successful liquidation receiving ma_token in return
        {
            ...
            let debt = DEBTS
                .load(&deps.storage, (cw20_debt_contract_addr.as_bytes(), &user_address))
                .unwrap();
    ...
    fn test_uncollateralized_loan_limits() {
        ...
        let info = mock_info("owner");
        let err =
            execute(deps.as_mut(), update_limit_env.clone(), info, update_limit_msg).unwrap_err();
    ...
    fn test_update_asset_collateral() {
        ...
        // User's health factor can't be less than 1 after disabling collateral
        {
            ...
            DEBTS
                .save(deps.as_mut().storage, (token_addr_3.as_bytes(), &user_addr), &debt)
                .unwrap();
    ...
    #[derive(Default, Debug)]
    struct TestUtilizationDeltaInfo {
        less_liquidity: Uint128,
        more_debt: Uint128,
        less_debt: Uint128,
        /// Used when passing less debt to compute deltas in the actual scaled amount
        user_current_debt_scaled: Uint128,
    }
