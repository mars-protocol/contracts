...
use cosmwasm_std::{
    attr, coin, coins, to_binary, Addr, BankMsg, CosmosMsg, Decimal, Response, StdResult, SubMsg,
    Uint128, WasmMsg,
};
...
pub fn test_liquidate() {
    ...
    let collateral_market_initial =
        th::init_market(deps.as_mut(), "collateral", &collateral_market);
    let debt_market_initial = th::init_market(deps.as_mut(), "debt", &debt_market);
    ...
    // Perform first successful liquidation receiving ma_token in return
    {
        ...
        let expected_debt_rates = th::get_expected_indices_and_rates(
            &debt_market_initial,
            block_time,
        ...
        assert_eq!(
            ...
            vec![
                ...
                SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
                    contract_addr: debt_market.ma_token_address.clone().to_string(),
        ...
        assert_eq!(
            ...
            vec![
                ...
                th::build_interests_updated_event("debt", &expected_debt_rates)
            ]
        );
    ...
    // and receive underlying collateral
    {
        ...
        let expected_debt_indices = th::get_expected_indices(&debt_market_before, block_time);
        ...
        let expected_collateral_rates = th::get_expected_indices_and_rates(
        ...
        assert_eq!(
            res.events,
            vec![
                th::build_interests_updated_event("collateral", &expected_collateral_rates),
                th::build_interests_updated_event("debt", &expected_debt_rates),
            ]
        );
    ...
    // Perform full liquidation receiving ma_token in return (user should not be able to use asset as collateral)
    {
        ...
        let expected_collateral_indices =
            th::get_expected_indices(&collateral_market_before, block_time);
...
fn test_liquidate_with_same_asset_for_debt_and_collateral() {
    ...
    let mut deps = th::setup(&[coin(available_liquidity.into(), "the_asset")]);
    ...
    let asset_market_initial = th::init_market(deps.as_mut(), "the_asset", &asset_market);
    ...
    // Perform partial liquidation receiving ma_token in return
    {
        ...
        // get expected indices and rates for debt market
        let expected_rates = th::get_expected_indices_and_rates(
            &asset_market_before,
            block_time,
            available_liquidity,
            th::TestUtilizationDeltaInfo {
        ...
        assert_eq!(
            ...
            vec![
                ...
                th::build_interests_updated_event("the_asset", &expected_rates)
            ]
        );
    ...
    // Perform partial liquidation receiving underlying asset in return
    {
        ...
        assert_eq!(
            res.events,
            vec![th::build_interests_updated_event("the_asset", &expected_rates),]
        );
    ...
    // Perform overpaid liquidation receiving ma_token in return
    {
        ...
        // get expected indices and rates for debt and collateral markets
        let expected_indices = th::get_expected_indices(&asset_market_initial, block_time);
...
fn test_underlying_asset_balance_check_when_transfer_to_liquidator() {
    ...
    let mut deps = th::setup(&[coin(collateral_liquidity, "collateral")]);
...
fn test_liquidate_if_collateral_disabled() {
    ...
    let collateral_market_initial_1 =
        th::init_market(deps.as_mut(), "collateral1", &collateral_market_1);
    let _collateral_market_initial_2 =
        th::init_market(deps.as_mut(), "collateral2", &collateral_market_2);
