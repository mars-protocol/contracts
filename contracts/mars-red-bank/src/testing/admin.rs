...
use mars_testing::{mock_dependencies, mock_env_at_block_time, mock_info};
...
fn test_proper_initialization() {
    ...
    let err = instantiate(
        ...
        InstantiateMsg {
            config: CreateOrUpdateConfig {
                close_factor: Some(Decimal::from_ratio(13u128, 10u128)), // invalid
                ..base_config.clone()
            },
        },
    )
    ...
    let res = instantiate(
        ...
        InstantiateMsg {
            config: CreateOrUpdateConfig {
                close_factor: Some(Decimal::from_ratio(1u128, 2u128)), // valid
                ..base_config
            },
        },
    )
...
fn test_update_asset_with_new_interest_rate_model_params() {
    ...
    let expected_indices = th::get_expected_indices(&market_before, 2_000_000);
...
fn test_update_asset_new_reserve_factor_accrues_interest_rate() {
    ...
    let mut deps = th::setup(&[coin(asset_liquidity.into(), "somecoin")]);
    ...
    let market_before = th::init_market(
        deps.as_mut(),
        "somecoin",
        &Market {
    ...
    // Indices should have been updated using previous interest rate
    let expected_indices = th::get_expected_indices(&market_before, 1_500_000);
    assert_eq!(new_market.liquidity_index, expected_indices.liquidity);
    assert_eq!(new_market.borrow_index, expected_indices.borrow);
