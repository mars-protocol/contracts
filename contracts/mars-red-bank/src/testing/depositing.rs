...
use mars_testing::{mock_env_at_block_time, mock_info};
...
fn test_deposit_native_asset() {
    ...
    let mut deps = th::setup(&[coin(initial_liquidity.into(), "somecoin")]);
    ...
    let market = th::init_market(deps.as_mut(), "somecoin", &mock_market);
...
fn test_deposit_on_behalf_of() {
    ...
    let mut deps = th::setup(&[coin(initial_liquidity, "somecoin")]);
