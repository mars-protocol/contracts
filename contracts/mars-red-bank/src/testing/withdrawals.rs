...
fn test_withdraw_and_send_funds_to_another_user() {
    ...
    let market_initial = th::init_market(deps.as_mut(), denom, &mock_market);
...
fn test_withdraw_if_health_factor_not_met() {
    ...
    let mut deps = th::setup(&[coin(initial_available_liquidity.into(), "token3")]);
    ...
    let market_1_initial = th::init_market(deps.as_mut(), "token1", &market_1);
    let market_2_initial = th::init_market(deps.as_mut(), "token2", &market_2);
    let market_3_initial = th::init_market(deps.as_mut(), "token3", &market_3);
...
fn test_withdraw_total_balance() {
    ...
    let initial_available_liquidity = Uint128::from(12000000u128);
    let mut deps = th::setup(&[coin(initial_available_liquidity.into(), "somecoin")]);
...
fn test_withdraw_without_existing_position() {
    ...
    th::init_market(deps.as_mut(), "somecoin", &market);
