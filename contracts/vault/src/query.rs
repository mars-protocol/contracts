...
use crate::{
    ...
    msg::VaultUnlock,
    state::{PerformanceFeeState, PERFORMANCE_FEE_STATE, UNLOCKS},
};
...
pub fn performance_fee(deps: Deps, _env: Env) -> ContractResult<PerformanceFeeState> {
    Ok(PERFORMANCE_FEE_STATE.load(deps.storage)?)
}
