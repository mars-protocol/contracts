...
export const mockVaultQueryKeys = {
  ...
  unlockingPositionsForAddr: (
    ...
    [
      {
        ...mockVaultQueryKeys.address(contractAddress)[0],
        method: 'unlocking_positions_for_addr',
        args,
      },
    ] as const,
  unlockingPosition: (contractAddress: string | undefined, args?: Record<string, unknown>) =>
    [
      { ...mockVaultQueryKeys.address(contractAddress)[0], method: 'unlocking_position', args },
    ] as const,
}
...
export interface MockVaultUnlockingPositionQuery<TData>
  extends MockVaultReactQuery<UnlockingPosition, TData> {
  args: {
    id: Uint128
  }
}
export function useMockVaultUnlockingPositionQuery<TData = UnlockingPosition>({
  client,
  args,
  options,
}: MockVaultUnlockingPositionQuery<TData>) {
  return useQuery<UnlockingPosition, Error, TData>(
    mockVaultQueryKeys.unlockingPosition(client?.contractAddress, args),
    () =>
      client
        ? client.unlockingPosition({
            id: args.id,
          })
...
export interface MockVaultUnlockingPositionsForAddrQuery<TData>
  extends MockVaultReactQuery<ArrayOfUnlockingPosition, TData> {
  args: {
    addr: string
  }
}
export function useMockVaultUnlockingPositionsForAddrQuery<TData = ArrayOfUnlockingPosition>({
  client,
  args,
  options,
}: MockVaultUnlockingPositionsForAddrQuery<TData>) {
  return useQuery<ArrayOfUnlockingPosition, Error, TData>(
    mockVaultQueryKeys.unlockingPositionsForAddr(client?.contractAddress, args),
    () =>
      client
        ? client.unlockingPositionsForAddr({
            addr: args.addr,
          })
        : Promise.reject(new Error('Invalid client')),
    { ...options, enabled: !!client && (options?.enabled != undefined ? options.enabled : true) },
...
export interface MockVaultWithdrawUnlockedMutation {
...
export function useMockVaultWithdrawUnlockedMutation(
  options?: Omit<
    UseMutationOptions<ExecuteResult, Error, MockVaultWithdrawUnlockedMutation>,
    'mutationFn'
  >,
) {
  return useMutation<ExecuteResult, Error, MockVaultWithdrawUnlockedMutation>(
    ({ client, msg, args: { fee, memo, funds } = {} }) =>
      client.withdrawUnlocked(msg, fee, memo, funds),
    options,
  )
}
export interface MockVaultRequestUnlockMutation {
  client: MockVaultClient
  args?: {
    fee?: number | StdFee | 'auto'
    memo?: string
...
export function useMockVaultRequestUnlockMutation(
  options?: Omit<
    UseMutationOptions<ExecuteResult, Error, MockVaultRequestUnlockMutation>,
    'mutationFn'
  >,
) {
  return useMutation<ExecuteResult, Error, MockVaultRequestUnlockMutation>(
    ({ client, args: { fee, memo, funds } = {} }) => client.requestUnlock(fee, memo, funds),
    options,
  )
}
