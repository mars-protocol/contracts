...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const wallet = yield DirectSecp256k1HdWallet.fromMnemonic(deployer_wallet_mnemonic, { prefix: PREFIX });
        ...
        const client = yield SigningCosmWasmClient.connectWithSigner(rpc_endpoint, wallet, clientOption);
        const accountBalance = yield client.getBalance(deployerAddress, OSMOSIS_DENOM);
        console.log(`uosmo account balance is: ${accountBalance.amount} (${Number(accountBalance.amount) / 1e6} OSMO)`);
        if (Number(accountBalance.amount) < 1000000 &&
            chain_id === 'osmo-test-4') {
        ...
        if (!network.redBankCodeId) {
            ...
            uploadResult = yield client.upload(deployerAddress, redBankWasm, 'auto');
        ...
        if (!network.addressProviderCodeId) {
            ...
            uploadResult = yield client.upload(deployerAddress, addressProviderWasm, 'auto');
        ...
        if (!network.maTokenCodeId) {
            ...
            uploadResult = yield client.upload(deployerAddress, maTokenWasm, 'auto');
        ...
        if (!network.incentivesCodeId) {
            ...
            uploadResult = yield client.upload(deployerAddress, incentivesWasm, 'auto');
        ...
        if (!network.oracleCodeId) {
            ...
            uploadResult = yield client.upload(deployerAddress, oracleWasm, 'auto');
        ...
        if (!network.protocolRewardsCollectorCodeId) {
            ...
            uploadResult = yield client.upload(deployerAddress, protocolRewardsCollectorWasm, 'auto');
        ...
        if (!network.addressProviderContractAddress) {
            ...
            const { contractAddress: addressProviderContractAddress } = yield client.instantiate(deployerAddress, network.addressProviderCodeId, msg, 'mars-address-provider', 'auto');
        ...
        if (!network.redBankContractAddress) {
            ...
            const { contractAddress: redBankContractAddress } = yield client.instantiate(deployerAddress, network.redBankCodeId, msg, 'mars-red-bank', 'auto');
        ...
        if (!network.incentivesContractAddress) {
            ...
            const { contractAddress: incentivesContractAddress } = yield client.instantiate(deployerAddress, network.incentivesCodeId, msg, 'mars-incentives', 'auto');
        ...
        if (!network.oracleContractAddress) {
            ...
            const { contractAddress: oracleContractAddress } = yield client.instantiate(deployerAddress, network.oracleCodeId, msg, 'mars-oracle', 'auto');
        ...
        if (!network.protocolRewardsCollectorContractAddress) {
            ...
            const { contractAddress: protocolRewardsCollectorContractAddress } = yield client.instantiate(deployerAddress, network.protocolRewardsCollectorCodeId, msg, 'mars-protocol-rewards-collector', 'auto');
            network.protocolRewardsCollectorContractAddress =
                protocolRewardsCollectorContractAddress;
            // set osmo : atom route
            // todo search for correct pool and create if does not exist.
            yield client.execute(deployerAddress, network.protocolRewardsCollectorContractAddress, {
                set_route: {
        ...
        if (!network.addressProviderUpdated) {
            ...
            for (const index in addressesToSet) {
                ...
                result = yield client.execute(deployerAddress, network.addressProviderContractAddress, { set_address: addressObject }, 'auto');
        ...
        console.log(`${chain_id} :: Address Provider config : `, yield client.queryContractSmart(network.addressProviderContractAddress, { config: {} }));
        /*********************************************************************************************************** */
        /*********************************************************************************************************** */
        ...
        if (!network.uosmoRedBankMarketInitialised) {
            ...
            result = yield client.execute(deployerAddress, network.redBankContractAddress, msg, 'auto');
            wasmEvent = result.logs[0].events.find((e) => e.type === 'wasm');
            console.info('The `wasm` event emitted by the contract execution:', wasmEvent);
            network.uosmoRedBankMarketInitialised = true;
            writeArtifact(network, chain_id);
        }
        console.log(`${chain_id} :: Red Bank config : `, yield client.queryContractSmart(network.redBankContractAddress, {
            config: {},
        }));
        console.log(`${chain_id} :: Red Bank markets list : `, yield client.queryContractSmart(network.redBankContractAddress, {
            markets: {},
        }));
        console.log(`${chain_id} :: Red Bank uosmo market config : `, yield client.queryContractSmart(network.redBankContractAddress, {
            market: { denom: OSMOSIS_DENOM },
        }));
        /*********************************************************************************************************** */
        /*********************************************************************************************************** */
        // STEP EXECUTE INITIALISE RED BANK ASSET
        ...
        console.log(`${chain_id} :: Red Bank uatom market config : `, yield client.queryContractSmart(network.redBankContractAddress, {
            market: {
        ...
        if (!network.uosmoOraclePriceSet) {
            ...
            result = yield client.execute(deployerAddress, network.oracleContractAddress, msg, 'auto');
            wasmEvent = result.logs[0].events.find((e) => e.type === 'wasm');
            console.info('The `wasm` event emitted by the contract execution:', wasmEvent);
        ...
        console.log(`${chain_id} :: uosmo oracle price :  ${yield client.queryContractSmart(network.oracleContractAddress, msg)}`);
        /*********************************************************************************************************** */
        /*********************************************************************************************************** */
        ...
        console.log(`${chain_id} :: uatom oracle price :  ${yield client.queryContractSmart(network.oracleContractAddress, msg)}`);
        if (!network.smokeTest) {
            ...
            result = yield client.execute(deployerAddress, network.redBankContractAddress, msg, 'auto', undefined, coins);
            wasmEvent = result.logs[0].events.find((e) => e.type === 'wasm');
            console.info('The `wasm` event emitted by the contract execution:', wasmEvent);
            msg = { user_position: { user_address: deployerAddress } };
            console.log(yield client.queryContractSmart(network.redBankContractAddress, msg));
            /*********************************************************************************************************** */
            /*********************************************************************************************************** */
...
function requiredEnvironmentVariables(envVars) {
