...
const REWARD_COLLECTOR_TIMEOUT_BLOCKS =
    process.env.REWARD_COLLECTOR_TIMEOUT_BLOCKS!
const REWARD_COLLECTOR_TIMEOUT_SECONDS =
    process.env.REWARD_COLLECTOR_TIMEOUT_SECONDS!
...
async function main() {
    ...
    console.log(
        `uosmo account balance is: ${accountBalance.amount} (${
            Number(accountBalance.amount) / 1e6
        } OSMO)`
    )
    if (
        Number(accountBalance.amount) < 1_000000 &&
        chain_id === 'osmo-test-4'
    ) {
    ...
    if (!network.redBankCodeId) {
        const redBankWasm = fs.readFileSync(
            ARTIFACTS_PATH + 'mars_red_bank.wasm'
        )
    ...
    if (!network.addressProviderCodeId) {
        const addressProviderWasm = fs.readFileSync(
            ARTIFACTS_PATH + 'mars_address_provider.wasm'
        )
    ...
    console.log(
        `${chain_id} :: Address Provider Code ID : ${network.addressProviderCodeId}`
    )
    ...
    if (!network.maTokenCodeId) {
        const maTokenWasm = fs.readFileSync(
            ARTIFACTS_PATH + 'mars_ma_token.wasm'
        )
    ...
    if (!network.incentivesCodeId) {
        const incentivesWasm = fs.readFileSync(
            ARTIFACTS_PATH + 'mars_incentives.wasm'
        )
        uploadResult = await client.upload(
            deployerAddress,
            incentivesWasm,
    ...
    console.log(
        `${chain_id} :: Incentives Code ID : ${network.incentivesCodeId}`
    )
    ...
    if (!network.oracleCodeId) {
        const oracleWasm = fs.readFileSync(
            ARTIFACTS_PATH + 'mars_oracle_osmosis.wasm'
        )
    ...
    if (!network.protocolRewardsCollectorCodeId) {
        const protocolRewardsCollectorWasm = fs.readFileSync(
            ARTIFACTS_PATH + 'mars_rewards_collector_osmosis.wasm'
        )
        uploadResult = await client.upload(
            deployerAddress,
            protocolRewardsCollectorWasm,
            'auto'
        )
    ...
    console.log(
        `${chain_id} :: Protocol Rewards Collector Code ID : ${network.protocolRewardsCollectorCodeId}`
    )
    ...
    if (!network.addressProviderContractAddress) {
        ...
        const { contractAddress: addressProviderContractAddress } =
    ...
    console.log(
        `${chain_id} :: Address Provider Contract Address : ${network.addressProviderContractAddress}`
    )
    ...
    if (!network.redBankContractAddress) {
        ...
        const { contractAddress: redBankContractAddress } =
            await client.instantiate(
                deployerAddress,
    ...
    console.log(
        `${chain_id} :: Red Bank Contract Address : ${network.redBankContractAddress}`
    )
    ...
    if (!network.incentivesContractAddress) {
        ...
        const { contractAddress: incentivesContractAddress } =
            await client.instantiate(
                deployerAddress,
    ...
    console.log(
        `${chain_id} :: Incentives Contract Address : ${network.incentivesContractAddress}`
    )
    ...
    if (!network.oracleContractAddress) {
        ...
        const { contractAddress: oracleContractAddress } =
            await client.instantiate(
                deployerAddress,
    ...
    console.log(
        `${chain_id} :: Oracle Contract Address : ${network.oracleContractAddress}`
    )
    ...
    if (!network.protocolRewardsCollectorContractAddress) {
        ...
        const { contractAddress: protocolRewardsCollectorContractAddress } =
            await client.instantiate(
                deployerAddress,
    ...
    if (!network.uosmoRedBankMarketInitialised) {
        ...
        result = await client.execute(
            deployerAddress,
            network.redBankContractAddress,
            msg,
            'auto'
        )
        wasmEvent = result.logs[0].events.find((e) => e.type === 'wasm')
    ...
    console.log(
        `${chain_id} :: uosmo oracle price :  ${await client.queryContractSmart(
            network.oracleContractAddress,
            msg
        )}`
    )
    ...
    console.log(
        `${chain_id} :: uatom oracle price :  ${await client.queryContractSmart(
            network.oracleContractAddress,
            msg
        )}`
    )
    ...
    if (!network.smokeTest) {
        ...
        console.log(
            await client.queryContractSmart(network.redBankContractAddress, msg)
        )
