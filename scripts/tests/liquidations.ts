...
async function testCollateralizedNativeLoan(
  ...
  txResult = await executeContract(
    terra,
    liquidator,
    redBank,
    {
  ...
  const uusdAmountLiquidatedTax = (
    await terra.utils.calculateTax(new Coin('uusd', uusdAmountLiquidated))
  ).amount.toNumber();
  if (liquidatorOverpays) {
...
async function testUncollateralizedNativeLoan(env: Env, borrower: Wallet, logger?: Logger) {
