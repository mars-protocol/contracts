...
async function testNative(env: Env, logger?: Logger) {
...

  {
    ...
    const expectedTreasuryUusdBalanceDifference = (
      await deductTax(terra, new Coin('uusd', protocolRewardsCollectorUusdBalanceDifference * TREASURY_FEE_SHARE))
    ).toNumber();
    const expectedSafetyFundUusdBalanceDifference = (
      await deductTax(terra, new Coin('uusd', protocolRewardsCollectorUusdBalanceDifference * SAFETY_FUND_FEE_SHARE))
    ).toNumber();
    ...
    const expectedStakingUusdBalanceDifference = (
      await deductTax(
        ...
        new Coin(
          'uusd',
          protocolRewardsCollectorUusdBalanceDifference * (1 - (TREASURY_FEE_SHARE + SAFETY_FUND_FEE_SHARE)),
        ),
      )
    ).toNumber();
...
(async () => {
  ...
  const protocolRewardsCollector = await deployContract(
    terra,
    deployer,
    '../artifacts/mars_protocol_rewards_collector.wasm',
    {
      config: {
        owner: deployer.key.accAddress,
        address_provider_address: addressProvider,
  ...
  await setAssetOraclePriceSource(terra, deployer, oracle, { native: { denom: 'uluna' } }, LUNA_USD_PRICE, logger);
  ...
  await setAssetOraclePriceSource(
    terra,
    deployer,
    oracle,
  ...
  await mintCw20(
    ...
    CW20_TOKEN_1_UUSD_PAIR_CW20_TOKEN_1_LP_AMOUNT,
    logger,
  );
