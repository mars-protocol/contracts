...
import { deployContract, executeContract, instantiateContract, Logger, queryContract, setGasAdjustment, setTimeoutDuration, sleep, toEncodedBinary, uploadContract, } from '../helpers.js';
import { borrowCw20, borrowNative, deductTax, depositCw20, depositNative, mintCw20, queryBalanceCw20, queryMaAssetAddress, queryBalanceNative, setAssetOraclePriceSource, approximateEqual, } from './test_helpers.js';
// CONSTS
...
const CW20_TOKEN_1_UUSD_PAIR_UUSD_LP_AMOUNT = 1000000000000;
const CW20_TOKEN_1_UUSD_PAIR_CW20_TOKEN_1_LP_AMOUNT = CW20_TOKEN_1_UUSD_PAIR_UUSD_LP_AMOUNT * CW20_TOKEN_USD_PRICE;
// TESTS
function testNative(env, logger) {
    return __awaiter(this, void 0, void 0, function* () {
        const { terra, deployer, provider, borrower, maUusd, redBank, protocolRewardsCollector, treasury, safetyFund, staking, } = env;
        {
            console.log('provider provides uusd');
            const maUusdBalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maUusd);
            strictEqual(maUusdBalanceBefore, 0);
            yield depositNative(terra, provider, redBank, 'uusd', USD_COLLATERAL_AMOUNT);
            const maUusdBalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maUusd);
        ...
        yield borrowNative(terra, borrower, redBank, 'uusd', Math.floor(USD_BORROW_AMOUNT), logger);
        {
            console.log('repay');
            const maUusdBalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maUusd);
            yield executeContract(terra, borrower, redBank, { repay_native: { denom: 'uusd' } }, { coins: `${Math.floor(USD_BORROW_AMOUNT)}uusd`, logger: logger });
            const maUusdBalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maUusd);
        ...
        }
        {
            ...
            const maUusdBalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maUusd);
            yield executeContract(terra, provider, redBank, {
                withdraw: {
        ...
        console.log('protocol rewards collector withdraws from the red bank');
        {
            ...
            const maUusdBalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maUusd);
            const uusdBalanceBefore = yield queryBalanceNative(terra, protocolRewardsCollector, 'uusd');
            // withdraw half of the deposited balance
            yield executeContract(terra, deployer, protocolRewardsCollector, {
            ...
            const maUusdBalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maUusd);
            const uusdBalanceAfter = yield queryBalanceNative(terra, protocolRewardsCollector, 'uusd');
            assert(maUusdBalanceAfter < maUusdBalanceBefore);
        ...
        }
        {
            ...
            let result = yield executeContract(terra, deployer, protocolRewardsCollector, { withdraw_from_red_bank: { asset: { native: { denom: 'uusd' } } } }, { logger: logger });
            const maUusdBalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maUusd);
            const uusdBalanceAfter = yield queryBalanceNative(terra, protocolRewardsCollector, 'uusd');
            assert(uusdBalanceAfter > uusdBalanceBefore);
        ...
        yield assert.rejects(executeContract(terra, deployer, protocolRewardsCollector, { distribute_protocol_rewards: { asset: { native: { denom: 'uusd' } } } }, { logger: logger }), (error) => {
        ...
        }, { logger: logger });
        {
            ...
            const protocolRewardsCollectorUusdBalanceBefore = yield queryBalanceNative(terra, protocolRewardsCollector, 'uusd');
            const treasuryUusdBalanceBefore = yield queryBalanceNative(terra, treasury, 'uusd');
            const safetyFundUusdBalanceBefore = yield queryBalanceNative(terra, safetyFund, 'uusd');
            const stakingUusdBalanceBefore = yield queryBalanceNative(terra, staking, 'uusd');
            yield executeContract(terra, deployer, protocolRewardsCollector, { distribute_protocol_rewards: { asset: { native: { denom: 'uusd' } } } }, { logger: logger });
            const protocolRewardsCollectorUusdBalanceAfter = yield queryBalanceNative(terra, protocolRewardsCollector, 'uusd');
            const treasuryUusdBalanceAfter = yield queryBalanceNative(terra, treasury, 'uusd');
            const safetyFundUusdBalanceAfter = yield queryBalanceNative(terra, safetyFund, 'uusd');
            const stakingUusdBalanceAfter = yield queryBalanceNative(terra, staking, 'uusd');
            ...
            const protocolRewardsCollectorUusdBalanceDifference = protocolRewardsCollectorUusdBalanceBefore - protocolRewardsCollectorUusdBalanceAfter;
            ...
            const expectedTreasuryUusdBalanceDifference = (yield deductTax(terra, new Coin('uusd', protocolRewardsCollectorUusdBalanceDifference * TREASURY_FEE_SHARE))).toNumber();
            const expectedSafetyFundUusdBalanceDifference = (yield deductTax(terra, new Coin('uusd', protocolRewardsCollectorUusdBalanceDifference * SAFETY_FUND_FEE_SHARE))).toNumber();
            const expectedStakingUusdBalanceDifference = (yield deductTax(terra, new Coin('uusd', protocolRewardsCollectorUusdBalanceDifference * (1 - (TREASURY_FEE_SHARE + SAFETY_FUND_FEE_SHARE))))).toNumber();
...
function testCw20(env, logger) {
    return __awaiter(this, void 0, void 0, function* () {
        const { terra, deployer, provider, borrower, cw20Token1, cw20Token2, maCw20Token1, redBank, protocolRewardsCollector, treasury, safetyFund, staking, } = env;
        // mint some tokens
        ...
        yield mintCw20(terra, deployer, cw20Token2, borrower.key.accAddress, CW20_TOKEN_2_COLLATERAL_AMOUNT, logger);
        {
            ...
            const maCwToken1BalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maCw20Token1);
            ...
            const maCwToken1BalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maCw20Token1);
        ...
        yield borrowCw20(terra, borrower, redBank, cw20Token1, CW20_TOKEN_1_BORROW_AMOUNT, logger);
        {
            console.log('repay');
            const maCwToken1BalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maCw20Token1);
            yield executeContract(terra, borrower, cw20Token1, {
                send: {
                    contract: redBank,
        ...
        console.log('protocol rewards collector withdraws from the red bank');
        {
            console.log('- specify an amount');
            const maCwToken1BalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maCw20Token1);
            const cwToken1BalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, cw20Token1);
            // withdraw half of the deposited balance
            yield executeContract(terra, deployer, protocolRewardsCollector, {
                withdraw_from_red_bank: {
                    asset: { cw20: { contract_addr: cw20Token1 } },
            ...
            const maCwToken1BalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maCw20Token1);
            const cwToken1BalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, cw20Token1);
            assert(maCwToken1BalanceAfter < maCwToken1BalanceBefore);
        ...
        }
        {
            ...
            const cwToken1BalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, cw20Token1);
            // withdraw remaining balance
            const result = yield executeContract(terra, deployer, protocolRewardsCollector, { withdraw_from_red_bank: { asset: { cw20: { contract_addr: cw20Token1 } } } }, { logger: logger });
            const maCwToken1BalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maCw20Token1);
            const cwToken1BalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, cw20Token1);
            assert(cwToken1BalanceAfter > cwToken1BalanceBefore);
            // withdrawing from the red bank triggers protocol rewards to be minted to the protocol rewards
        ...
        yield assert.rejects(executeContract(terra, deployer, protocolRewardsCollector, { distribute_protocol_rewards: { asset: { cw20: { contract_addr: cw20Token1 } } } }, { logger: logger }), (error) => {
        ...
        yield executeContract(terra, deployer, protocolRewardsCollector, { swap_asset_to_uusd: { offer_asset_info: { token: { contract_addr: cw20Token1 } } } }, { logger: logger });
        console.log('enable uusd for distribution');
        yield executeContract(terra, deployer, protocolRewardsCollector, {
            update_asset_config: {
                asset: { native: { denom: 'uusd' } },
...
function testLiquidateNative(env, logger) {
    return __awaiter(this, void 0, void 0, function* () {
        const { terra, deployer, provider, borrower, maUluna, maUusd, redBank, protocolRewardsCollector } = env;
        ...
        yield borrowNative(terra, provider, redBank, 'uluna', Math.floor(LUNA_COLLATERAL_AMOUNT / 10), logger);
        ...
        const maUusdBalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maUusd);
        const maUlunaBalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maUluna);
        yield executeContract(terra, liquidator, redBank, {
            liquidate_native: {
        ...
        const maUusdBalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maUusd);
        const maUlunaBalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maUluna);
        assert(maUusdBalanceAfter > maUusdBalanceBefore);
        assert(maUlunaBalanceAfter > maUlunaBalanceBefore);
    });
}
function testLiquidateCw20(env, logger) {
    return __awaiter(this, void 0, void 0, function* () {
        const { terra, deployer, provider, borrower, maCw20Token1, maCw20Token2, cw20Token1, cw20Token2, redBank, protocolRewardsCollector, } = env;
        const liquidator = deployer;
        // mint some tokens
        yield mintCw20(terra, deployer, cw20Token1, provider.key.accAddress, CW20_TOKEN_1_COLLATERAL_AMOUNT, logger);
        ...
        yield borrowCw20(terra, provider, redBank, cw20Token2, Math.floor(CW20_TOKEN_1_BORROW_AMOUNT / 10), logger);
        console.log("liquidator waits until the borrower's health factor is < 1, then liquidates");
        // wait until the borrower can be liquidated
        let tries = 0;
        let maxTries = 10;
        ...
        // get the protocol rewards collector balances before the borrower is liquidated
        const maCwToken1BalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maCw20Token1);
        const maCwToken2BalanceBefore = yield queryBalanceCw20(terra, protocolRewardsCollector, maCw20Token2);
        yield executeContract(terra, liquidator, cw20Token1, {
            send: {
                contract: redBank,
        ...
        // get the protocol rewards collector balances after the borrower is liquidated
        const maCwToken1BalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maCw20Token1);
        const maCwToken2BalanceAfter = yield queryBalanceCw20(terra, protocolRewardsCollector, maCw20Token2);
        assert(maCwToken1BalanceAfter > maCwToken1BalanceBefore);
...
(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const protocolRewardsCollector = yield deployContract(terra, deployer, '../artifacts/mars_protocol_rewards_collector.wasm', {
        config: {
            owner: deployer.key.accAddress,
            address_provider_address: addressProvider,
    ...
    const maCw20Token1 = yield queryMaAssetAddress(terra, redBank, { cw20: { contract_addr: cw20Token1 } });
    // cw20token2
    yield executeContract(terra, deployer, redBank, {
        init_asset: {
    ...
    let result = yield executeContract(terra, deployer, astroportFactory, {
        create_pair: {
            pair_type: { xyk: {} },
            asset_infos: [{ token: { contract_addr: cw20Token1 } }, { native_token: { denom: 'uusd' } }],
        },
    }, { logger: logger });
    const cw20Token1UusdPair = result.logs[0].eventsByType.wasm.pair_contract_addr[0];
    yield mintCw20(terra, deployer, cw20Token1, deployer.key.accAddress, CW20_TOKEN_1_UUSD_PAIR_CW20_TOKEN_1_LP_AMOUNT, logger);
    yield executeContract(terra, deployer, cw20Token1, {
        increase_allowance: {
            spender: cw20Token1UusdPair,
            amount: String(CW20_TOKEN_1_UUSD_PAIR_CW20_TOKEN_1_LP_AMOUNT),
        },
    }, { logger: logger });
    yield executeContract(terra, deployer, cw20Token1UusdPair, {
        provide_liquidity: {
            assets: [
                {
