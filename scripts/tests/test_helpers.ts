...
import { executeContract, Logger, queryContract, sleep, toEncodedBinary } from '../helpers.js';
...
export async function queryBalanceCw20(terra: LCDClient, userAddress: string, contractAddress: string) {
...
export async function queryBalanceNative(terra: LCDClient, address: string, denom: string) {
...
export async function queryMaAssetAddress(terra: LCDClient, redBank: string, asset: Asset): Promise<string> {
...
export async function getBlockHeight(terra: LCDClient, txResult: BlockTxBroadcastResult) {
  await sleep(100);
  const txInfo = await terra.tx.txInfo(txResult.txhash);
  return txInfo.height;
}

export async function getTxTimestamp(terra: LCDClient, result: BlockTxBroadcastResult) {
  const txInfo = await terra.tx.txInfo(result.txhash);
  return Date.parse(txInfo.timestamp) / 1000; // seconds
}

export async function waitUntilBlockHeight(terra: LCDClient, blockHeight: number) {
  const maxTries = 10;
  let tries = 0;
  let backoff = 1;
  while (true) {
...
export function approximateEqual(actual: number, expected: number, tol: number) {
  try {
