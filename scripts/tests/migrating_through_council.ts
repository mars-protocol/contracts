...
(async () => {
  ...
  const council = await instantiateContract(
    terra,
    deployer,
    councilCodeId,
    {
  ...
  const counterVer1CodeId = await uploadContract(
    terra,
    deployer,
    join(MARS_MOCKS_ARTIFACTS_PATH, 'counter_version_one.wasm'),
  );
  const counterVer1 = await instantiateContract(
    terra,
    deployer,
    counterVer1CodeId,
    { owner: deployer.key.accAddress },
  ...
  // migrate `counter` contract
  {
    ...
    const counterVer2CodeId = await uploadContract(
      terra,
      deployer,
      join(MARS_MOCKS_ARTIFACTS_PATH, 'counter_version_two.wasm'),
    );
    ...
    let txResult = await executeContract(
      terra,
      john,
      mars,
      {
    ...
    await executeContract(
      ...
      deployer,
      council,
      { end_proposal: { proposal_id: johnProposalId } },
      { logger: logger },
    );
    ...
    await executeContract(
      ...
      deployer,
      council,
      { execute_proposal: { proposal_id: johnProposalId } },
      { logger: logger },
    );
  ...
  // migrate `council` contract
  {
    ...
    const councilVer2CodeId = await uploadContract(
      terra,
      deployer,
      join(MARS_MOCKS_ARTIFACTS_PATH, 'counter_version_two.wasm'),
    );
