...
async function main() {
  ...
  if (
    !deployConfig.stakingInitMsg.config.astroport_factory_address ||
  ...
  await executeContract(terra, wallet, deployConfig.minterProxyContractAddress, {
    update_admins: {
      admins: isTestnet ? [wallet.key.accAddress, councilContractAddress] : [councilContractAddress],
    },
  });
  console.log(
    'Council set to MARS token minter admin role: ',
    await queryContract(terra, deployConfig.minterProxyContractAddress, { admin_list: {} }),
  );
  ...
  const xMarsTokenCodeId = await uploadContract(terra, wallet, join(MARS_ARTIFACTS_PATH, 'mars_xmars_token.wasm'));
  ...
  const maTokenCodeId = await uploadContract(terra, wallet, join(MARS_ARTIFACTS_PATH, 'mars_ma_token.wasm'));
  ...
  console.log(
    'Protocol Rewards Collector owner successfully changed: ',
    await queryContract(terra, protocolRewardsCollectorContractAddress, { config: {} }),
  );
  ...
  const oracleContractAddress = await deployContract(terra, wallet, join(MARS_ARTIFACTS_PATH, 'mars_oracle.wasm'), {
    owner: wallet.key.accAddress,
  });
  ...
  console.log(
    'Address Provider config successfully setup: ',
    await queryContract(terra, addressProviderContractAddress, { config: {} }),
  );
  ...
  await setupRedBank(terra, wallet, redBankContractAddress, {
  ...
  if (deployConfig.mirFarmingStratContractAddress) {
    ...
    console.log(
      `Uncollateralized loan limit for contract ${deployConfig.mirFarmingStratContractAddress} (Fields MIR-UST):`,
      await queryContract(terra, redBankContractAddress, {
  ...
  if (deployConfig.ancFarmingStratContractAddress) {
    ...
    console.log(
      `Uncollateralized loan limit for contract ${deployConfig.ancFarmingStratContractAddress} (Fields ANC-UST):`,
      await queryContract(terra, redBankContractAddress, {
        uncollateralized_loan_limit: {
          user_address: deployConfig.ancFarmingStratContractAddress,
          asset: { native: { denom: 'uusd' } },
  ...
  if (deployConfig.marsFarmingStratContractAddress) {
    ...
    console.log(
      `Uncollateralized loan limit for contract ${deployConfig.marsFarmingStratContractAddress} (Fields ANC-UST):`,
      await queryContract(terra, redBankContractAddress, {
        uncollateralized_loan_limit: {
          user_address: deployConfig.marsFarmingStratContractAddress,
          asset: { native: { denom: 'uusd' } },
  ...
  console.log(
    'Red Bank config successfully updated: ',
    await queryContract(terra, redBankContractAddress, { config: {} }),
  );
}
