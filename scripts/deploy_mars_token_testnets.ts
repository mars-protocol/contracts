...
async function main() {
  ...
  const marsTokenContractAddress = await deployContract(terra, wallet, join(MARS_ARTIFACTS_PATH, 'cw20_base.wasm'), {
    name: 'Mars',
    symbol: 'MARS',
    decimals: 6,
  ...
  const balanceResponse = await queryContract(terra, marsTokenContractAddress, {
    balance: {
      address: wallet.key.accAddress,
    },
  });
